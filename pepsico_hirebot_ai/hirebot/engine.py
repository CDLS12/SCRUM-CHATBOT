...
class HireBotEngine:
    ...
    def reply(self, user_message: str, state: BotState) -> Tuple[str, BotState]:
        ...
        if not text:
            return "Escribe una solicitud o usa el botón de ejemplo para iniciar.", state
        ...
        if state.awaiting_field:
            ...
            if parsed_value is None:
                notes.append(note or f"No pude registrar {FIELD_LABELS[state.awaiting_field]}.")
                parsed_focus = True
            else:
    ...
    def _extract_fields(self, text: str) -> Tuple[Dict[str, Any], List[str]]:
        ...
        m = re.search(r"(?:ubicacion|ubicación|ciudad|localidad)\s*(?:es|:|en|de)?\s*([a-záéíóúñü .]+?)(?=,|\.| modalidad | con | salario | sueldo | fecha |$)", raw, re.I)
        if m:
            updates["ubicacion"] = clean_text(m.group(1))
            
        # Sede
        m = re.search(r"(?:sede)\s*(?:es|:|en|de)?\s*([a-záéíóúñü0-9 .-]+?)(?=,|\.| modalidad | con | salario | sueldo | fecha |$)", raw, re.I)
        if m:
            updates["sede"] = clean_text(m.group(1))
            
        # Centro de costos
        m = re.search(r"(?:centro de costos|centro costos|cost center)\s*(?:es|:)?\s*([a-zA-Z0-9._/-]+)", raw, re.I)
        if m:
            updates["centro_costos"] = clean_text(m.group(1))
    ...
    def _parse_value_for_field(self, field_name: str, text: str) -> Tuple[Any, str]:
        ...
        if field_name in ["solicitante", "jefe_directo"]:
            is_valid, reason = validate_person_name(text)
            
            if not is_valid:
                return None, f"{FIELD_LABELS[field_name]} no es válido. {reason}"
            name = clean_name(text)
            return name, f"Registré {FIELD_LABELS[field_name]}: {name}."

        if field_name == "justificacion":
            value = clean_text(text)
            return value, f"Registré {FIELD_LABELS[field_name]}: {value}."

        if field_name in ["puesto", "area", "ubicacion", "sede", "centro_costos", "seniority", "tipo_contrato", "modalidad"]:
    ...
    def _welcome_response(self) -> str:
        return (
            ...
            "También puedes escribir `ejemplo` para cargar un ejemplo completo."
        )
    ...
    def _help_response(self) -> str:
        return (
            ...
            "4. **Generar un folio de solicitud** para guardar el registro.\n\n"
            "Comandos útiles: `ejemplo`, `resumen`, `confirmar`, `cancelar`."
        )
    ...
    def _ask_next_question(self, field_name: str, draft: Dict[str, Any], notes: List[str], warnings: List[str]) -> str:
        ...
        for n in unique(notes):
            n_low = normalize(n)

            if any(word in n_low for word in [
                "no pude",
                "no es valido",
                "no es válida",
                "no es valido",
    ...
    def _question_for_field(self, field_name: str) -> str:
        questions = {
            ...
            "sede": "¿Cuál es la sede específica? Ejemplo: Corporativo Monterrey, Planta Apodaca o Santa Fe.",
            "centro_costos": "¿Cuál es el centro de costos asociado a la vacante? Ejemplo: CC-2045 o 450123.",
            ...
            "fecha_inicio": "¿Cuál es la fecha deseada de inicio? Ejemplo: 15 de octubre del 2026.",
    ...
    def _extract_date(self, text: str) -> Optional[date]:
        ...
        # 15 octubre 2026
        m = re.search(
            rf"\b(\d{{1,2}})\s*(?:de\s+)?({month_names})\s*(?:del|de)?\s*(\d{{2,4}})\b",
            low,
            re.I,
        )
        ...
        if m:
            try:
                ...
                # Si la fecha ya pasó este año, asumimos el siguiente año
                if parsed < today:
    ...
    def demo_data(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "modalidad": "híbrido",
            "ubicacion": "Monterrey",
            "sede": "Corporativo Monterrey",
            "centro_costos": "CC-2045",
            "salario_min": 30000,
            "salario_max": 45000,
...
def validate_person_name(value: str) -> Tuple[bool, str]:
    ...
    if normalized in invalid_values:
        return False, "Debes escribir un nombre real, no un valor genérico."

    if re.fullmatch(r"\d+", raw):
        return False, "Un nombre no puede ser solamente números."

    if re.search(r"\d", raw):
        return False, "Un nombre no debe contener números."

    only_letters = re.sub(r"[^A-Za-zÁÉÍÓÚÜÑáéíóúüñ]", "", raw)

    if len(only_letters) < 3:
        return False, "El nombre debe contener al menos 3 letras."

    if not re.fullmatch(r"[A-Za-zÁÉÍÓÚÜÑáéíóúüñ .'-]+", raw):
        return False, "El nombre solo debe contener letras, espacios, puntos, guiones o apóstrofes."
